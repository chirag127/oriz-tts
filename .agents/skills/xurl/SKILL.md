...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by Hermes tool subprocesses; see the Docker HOME pitfall in the "One-Time User Setup" section below.
...
3. If auth is missing entirely, stop and direct the user to the "One-Time User Setup" section above — do NOT attempt to register apps or pass secrets yourself.
...
<!-- Merged from development/xurl + social-media/xurl 2026-07-11 -->
