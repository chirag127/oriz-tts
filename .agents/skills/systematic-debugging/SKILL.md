...
description: "4-phase root cause debugging: understand bugs before fixing. Use when encountering any bug, test failure, or unexpected behavior, before proposing fixes."
version: 1.1.0
...
Detail + checklists + tool actions: [`references/phase-1-root-cause.md`](references/phase-1-root-cause.md).
...
**Example (multi-layer system with env/secrets):**
...
Detail: [`references/phase-2-pattern-analysis.md`](references/phase-2-pattern-analysis.md).
...
Detail: [`references/phase-3-hypothesis.md`](references/phase-3-hypothesis.md).

> **NOTE — CONTRADICTION between sources:**
>
> - **v1.1 (Hermes/development):** Generate **3–5 ranked falsifiable hypotheses** before testing any; rank by likelihood and cheapness to falsify; show ranked list to user if present.
> - **v1.0 (obra/superpowers flat):** Form a **single hypothesis**; state clearly "I think X because Y"; write it down.
>
> Both approaches are preserved below. The ranked approach is from v1.1 and is generally preferred for complex bugs; the single-hypothesis approach is faster for simple, well-understood bugs.

### 1a. Form Ranked Falsifiable Hypotheses (v1.1 — preferred for complex bugs)
...
- If the user is present, show the ranked list before testing. If the user is AFK, proceed with your ranking.

### 1b. Form Single Hypothesis (v1.0 — valid for simple/clear bugs)
...
Detail: [`references/phase-4-implementation.md`](references/phase-4-implementation.md).
...
| --------------------- | ------------------------------------------------------------------------------ | ---------------------------- |
| **1. Root Cause**     | Read errors, build tight loop, check changes, gather evidence, trace data flow | Understand WHAT and WHY      |
| **2. Pattern**        | Minimize repro, find working examples, compare, identify differences           | Know what's different        |
| **3. Hypothesis**     | Form ranked hypotheses, test minimally, one variable at a time                 | Confirmed or new hypothesis  |
...
Detail: [`references/hermes-integration.md`](references/hermes-integration.md).
...
<!-- Ported from Hermes 2026-07-09; merged with obra/superpowers flat canonical -->
