# Submission — Anonymization, Compilation, Format Conversion, arXiv, Code

Final checks, formatting, submission logistics.
...
**If compilation fails**: Parse `.log` for the first error. Common fixes:
...
When cutting: move proofs to appendix, condense related work, combine tables, use subfigures.
...
## Step 7.8: Camera-Ready Preparation
...
Posting to arXiv is standard in ML but has timing and anonymity considerations.
...
| Paper already on arXiv, submitting to new venue | Acceptable at most venues. Do NOT update arXiv version during review with changes referencing reviews. |
