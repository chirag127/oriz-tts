> Folded from skill `code-research` on 2026-07-08 during skill-compact merge.

---
name: code-research
description: Research open-source repositories to understand how something is built or works. Use when the user asks how an external library/framework implements a feature, when you need to understand an external dependency's internals before integrating, when comparing how different public repos solve a problem, or when hunting for usage examples in a GitHub repo. Fires `github_codebase_search` with 2-4 parallel angles per question.
metadata:
  author: morph
  version: "0.1.0"
