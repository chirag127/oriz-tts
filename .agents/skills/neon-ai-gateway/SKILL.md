...
- **One API for all models** — Frontier and open-source models behind a single endpoint, addressed by their catalog ID (e.g. `Codex-sonnet-4-6`, `gpt-5-mini`, `gemini-2-5-flash`).
...
const result = streamText({
  model: neon("gpt-5-mini"), // or Codex-sonnet-4-6, gemini-2-5-flash, ...
  messages,
  tools: {
...
const { text } = await generateText({
  model: neon("Codex-haiku-4-5"), // or gpt-5-3-codex, gemini-2-5-flash, ...
...
const res = await client.responses.create({
  model: "gpt-5-mini", // swap to Codex-sonnet-4-6, gemini-2-5-flash, ...
  input: "What is Neon?",
});
```
...
Use a model's catalog ID directly in the `model` field — e.g. `Codex-sonnet-4-6`, `gpt-5-mini`, `gemini-2-5-flash`. No provider prefix is needed. To look up the exact identifiers the gateway serves, which underlying model each maps to, and their context windows, pricing, and capabilities, use any of:
