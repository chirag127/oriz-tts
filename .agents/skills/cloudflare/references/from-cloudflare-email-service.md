> Folded from skill `cloudflare-email-service` on 2026-07-08 during skill-compact merge.
...
1. **Domain onboarded?** Run `npx wrangler email sending list` to see which domains have email sending enabled. If the domain isn't listed, run `npx wrangler email sending enable userdomain.com` or see [cli-and-mcp.md](from-cloudflare-email-service-refs/cli-and-mcp.md) for full setup instructions.
...
|--------------|------|-----------|
| **Send emails from a Cloudflare Worker** | Workers binding (no API keys needed) | [sending.md](from-cloudflare-email-service-refs/sending.md) |
| **Send emails from an AI agent built with [Cloudflare Agents SDK](https://developers.cloudflare.com/agents/)** | `onEmail()` + `replyToEmail()` in Agent class | [sending.md](from-cloudflare-email-service-refs/sending.md) |
| **Send emails from an external app or agent** (Node.js, Go, Python, etc.) | REST API with Bearer token | [rest-api.md](from-cloudflare-email-service-refs/rest-api.md) |
| **Send emails from a coding agent** (Claude Code, Cursor, Copilot, etc.) | MCP tools, wrangler CLI, or REST API | [cli-and-mcp.md](from-cloudflare-email-service-refs/cli-and-mcp.md) |
| **Receive and process incoming emails** (Email Routing) | Workers `email()` handler | [routing.md](from-cloudflare-email-service-refs/routing.md) |
| **Set up Email Sending or Email Routing** | `wrangler email sending enable` / `wrangler email routing enable`, or Dashboard | [cli-and-mcp.md](from-cloudflare-email-service-refs/cli-and-mcp.md) |
| **Improve deliverability, avoid spam folders** | Authentication, content, compliance | [deliverability.md](from-cloudflare-email-service-refs/deliverability.md) |
...
The binding is recommended for Workers — no API keys needed. If a user specifically requests the REST API from within a Worker (e.g., they already have an API token workflow), that works too — see [rest-api.md](from-cloudflare-email-service-refs/rest-api.md).

See [sending.md](from-cloudflare-email-service-refs/sending.md) for the full API, batch sends, attachments, custom headers, restricted bindings, and Agents SDK integration.
...
See [rest-api.md](from-cloudflare-email-service-refs/rest-api.md) for curl examples, response format, and error handling.
...
- **from-cloudflare-email-service-refs/sending.md** — Workers binding API, attachments, Agents SDK email. For Workers or Agents SDK.
- **from-cloudflare-email-service-refs/rest-api.md** — REST endpoint, curl examples, error handling. For apps NOT on Workers.
- **from-cloudflare-email-service-refs/routing.md** — Inbound `email()` handler, forwarding, replying, parsing. For receiving emails.
- **from-cloudflare-email-service-refs/cli-and-mcp.md** — Domain setup, wrangler commands, MCP tools. For first-time setup.
- **from-cloudflare-email-service-refs/deliverability.md** — SPF/DKIM/DMARC, bounces, suppressions, best practices.
